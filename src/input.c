...
static void add_hotkeys_to_compositor(GromitData *data) {
	...
	*/
	...
	modifier_hotkey_option[18] = "<Alt>";
	modifier_hotkey_option[19] = data->menu_keyval;
	modifier_hotkey_option[20] = "--menutoggle";
  ...
  if(devdata)
    {
	  ...
	  {
        g_print("acquiring grab");
		acquire_grab (data, devdata->device);
		g_print("acquired grab");
	  }
    }
  else
		     ...
		     gpointer     func_data)
{
  ...
  if (event->type == GDK_KEY_PRESS &&
      (event->hardware_keycode == data->undo_keycode || event->hardware_keycode == data->menu_keycode))
    {
		g_print("\nkey pressed\n");
      if(data->debug)
